...
public sealed class ContentsClient<TEntity, TData> : SquidexClientBase, IContentsClient<TEntity, TData> where TEntity : Content<TData> where TData : class, new()
{
    ...
    public Task<ContentsResult<TEntity, TData>> GetAsync(ContentQuery? query = null, QueryContext? context = null,
         CancellationToken ct = default)
    {
        return RequestJsonAsync<ContentsResult<TEntity, TData>>(
            HttpMethod.Get,
            BuildUrl(string.Empty, true, query?.ToQuery(true, Options), context),
            null,
            context,
            ct);
    }
         ...
         CancellationToken ct = default)
    {
        ...
        return RequestJsonAsync<TEntity>(
            HttpMethod.Get,
            BuildUrl($"{Uri.EscapeDataString(id)}", true, null, context),
            null,
            context,
            ct);
    }
         ...
         CancellationToken ct = default)
    {
        ...
        return RequestJsonAsync<TEntity>(
            HttpMethod.Get,
            BuildUrl($"{Uri.EscapeDataString(id)}", true, Query.Create().Append("version", version), context),
            null,
            context,
            ct);
    }
         ...
         CancellationToken ct = default)
    {
        ...
        return RequestJsonAsync<ContentsResult<TEntity, TData>>(
            HttpMethod.Get,
            BuildUrl($"{Uri.EscapeDataString(id)}/referencing", true, query?.ToQuery(true, Options), context),
            null,
            context,
            ct);
    }
         ...
         CancellationToken ct = default)
    {
        ...
        return RequestJsonAsync<ContentsResult<TEntity, TData>>(
            HttpMethod.Get,
            BuildUrl($"{Uri.EscapeDataString(id)}/references", true, query?.ToQuery(true, Options), context),
            null,
            context,
            ct);
    }
         ...
         CancellationToken ct = default)
    {
        ...
        return RequestJsonAsync<TEntity>(
            HttpMethod.Post,
            BuildUrl(string.Empty, false, options.ToQuery()),
            data.ToContent(Options),
            context,
            ct);
    }
         ...
         CancellationToken ct = default)
    {
        ...
        return RequestJsonAsync<TEntity>(
            HttpMethod.Post, BuildUrl($"{Uri.EscapeDataString(id)}/draft", false),
            null,
            context,
            ct);
    }
         ...
         CancellationToken ct = default)
    {
        ...
        return RequestJsonAsync<TEntity>(
            HttpMethod.Put,
            BuildUrl($"{Uri.EscapeDataString(id)}/status", false),
            request.ToContent(Options),
            context,
            ct);
    }
         ...
         CancellationToken ct = default)
    {
        ...
        return RequestJsonAsync<TEntity>(
            HttpMethodEx.Patch,
            BuildUrl($"{Uri.EscapeDataString(id)}", false),
            patch.ToContent(Options),
            context,
            ct);
    }
         ...
         CancellationToken ct = default)
    {
        ...
        return RequestJsonAsync<TEntity>(
            HttpMethod.Post,
            BuildUrl($"{Uri.EscapeDataString(id)}", false, options.ToQuery()),
            data.ToContent(Options),
            context,
            ct);
    }
         ...
         CancellationToken ct = default)
    {
        ...
        return await RequestJsonAsync<TEntity>(
            HttpMethod.Put,
            BuildUrl($"{Uri.EscapeDataString(id)}", false, options.ToQuery()),
            data.ToContent(Options),
            context,
        ...
        CancellationToken ct = default)
    {
        ...
        return await RequestJsonAsync<TEntity>(
            HttpMethod.Put,
            BuildUrl($"{Uri.EscapeDataString(id)}/defaults", false, options.ToQuery()),
            null,
            context,
            ct);
    }
         ...
         CancellationToken ct = default)
    {
        ...
        return RequestJsonAsync<TEntity>(
            HttpMethod.Delete,
            BuildUrl($"{Uri.EscapeDataString(id)}/draft", false),
            null,
            context,
            ct);
    }
         ...
         CancellationToken ct = default)
    {
        ...
        return RequestJsonAsync<List<BulkResult>>(
            HttpMethod.Post,
            BuildUrl("bulk", false),
            update.ToContent(Options),
            null,
            ct);
    }

    private string BuildUrl(string path, bool isGet, Query? q = null, QueryContext? context = null)
    {
        string result;
        if (ShouldUseCDN(isGet, context))
        {
            result = $"{Options.ContentCDN}{Options.AppName}/{SchemaName}/{path}{q}";
        }
        else
        {
            result = $"api/content/{Options.AppName}/{SchemaName}/{path}{q}";
        }

        return result;
    }

    private bool ShouldUseCDN(bool isGet, QueryContext? context)
    {
        return isGet && !string.IsNullOrWhiteSpace(Options.ContentCDN) && context?.IsNotUsingCDN != true;
    }
}
